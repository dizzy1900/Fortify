...
export async function POST(request: NextRequest, { params }: { params: Promise<{ policyVersionId: string }> }) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const { policyVersionId } = await params; return Response.json(await getProductionProgrammeAnalyticsService().reviewAnalyticsPolicy(principal.authorization, { ...(await request.json()), policyVersionId }), { status: 201 }); }
