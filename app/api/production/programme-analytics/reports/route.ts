...
export async function POST(request: NextRequest) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); return Response.json(await getProductionProgrammeAnalyticsService().generateReport(principal.authorization, await request.json()), { status: 201 }); }
