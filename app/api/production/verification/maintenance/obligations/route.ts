...
export async function POST(request: NextRequest) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const service = getProductionVerificationService(); return Response.json(await service.createMaintenanceObligation(principal.authorization, await request.json() as Parameters<typeof service.createMaintenanceObligation>[1]), { status: 201 }); }
