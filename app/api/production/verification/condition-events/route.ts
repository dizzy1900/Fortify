...
export async function POST(request: NextRequest) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const service = getProductionVerificationService(); return Response.json(await service.recordPropertyConditionEvent(principal.authorization, await request.json() as Parameters<typeof service.recordPropertyConditionEvent>[1]), { status: 201 }); }
