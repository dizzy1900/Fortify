...
export async function POST(request: NextRequest, { params }: { params: Promise<{ versionId: string }> }) {
  try {
    ...
    const body = await request.json() as { decision: "approved" | "changes_requested"; note: string; sourcePinsChecked: boolean };
    return Response.json(await getProductionResiliencePlanningService().reviewProfileVersion(principal.authorization, { profileVersionId: versionId, ...body }), { status: 201 });
