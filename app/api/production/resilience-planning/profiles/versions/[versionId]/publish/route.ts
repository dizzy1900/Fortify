...
export async function POST(request: NextRequest, { params }: { params: Promise<{ versionId: string }> }) {
  try {
    ...
    return Response.json(await getProductionResiliencePlanningService().publishProfileVersion(principal.authorization, { profileVersionId: versionId, ...body }), { status: 201 });
