...
export async function POST(request: NextRequest) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const service = getProductionModelRecognitionService(); return Response.json(await service.registerRecognitionOrganization(principal.authorization, await request.json() as Parameters<typeof service.registerRecognitionOrganization>[1]), { status: 201 }); }
