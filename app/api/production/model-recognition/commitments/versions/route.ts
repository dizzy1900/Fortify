...
export async function POST(request: NextRequest) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const service = getProductionModelRecognitionService(); return Response.json(await service.createCommitmentVersion(principal.authorization, await request.json() as Parameters<typeof service.createCommitmentVersion>[1]), { status: 201 }); }
