...
export async function POST(request: NextRequest, { params }: { params: Promise<{ versionId: string }> }) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const service = getProductionModelRecognitionService(); const { versionId } = await params; const body = await request.json() as Omit<Parameters<typeof service.reviewCommitmentVersion>[1], "commitmentVersionId">; return Response.json(await service.reviewCommitmentVersion(principal.authorization, { ...body, commitmentVersionId: versionId }), { status: 201 }); }
