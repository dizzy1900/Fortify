...
export async function POST(request: NextRequest, { params }: { params: Promise<{ versionId: string }> }) {
  try { requireProductionRuntime(); const principal = await resolveRequestPrincipal(request); const service = getProductionModelRecognitionService(); const { versionId } = await params; const body = await request.json() as Omit<Parameters<typeof service.reviewModelVersion>[1], "modelVersionId">; return Response.json(await service.reviewModelVersion(principal.authorization, { ...body, modelVersionId: versionId }), { status: 201 }); }
