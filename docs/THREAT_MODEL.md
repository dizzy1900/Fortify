...
| Session theft or fixation  | opaque hashed sessions, secure HttpOnly SameSite cookie, expiry/revocation, OIDC state/nonce/PKCE  | live IdP/MFA and session anomaly alerting pending          |
| CSRF/clickjacking/XSS      | origin/fetch-metadata checks, frame denial, nonce CSP, contextual React escaping                   | independent penetration test pending                       |
