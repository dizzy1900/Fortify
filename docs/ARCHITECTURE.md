...
The target system is a California-first Resilience Investment and Insurance Recognition OS. The normalized M0–M11 tree now includes governed sources, planning/funding/execution, independent verification, external-model/commitment mapping, recognition submission and separate outcomes, programme analytics, and governed provider custody. It remains an undeployed local implementation until M12 and the separate external gates are satisfied.
...
`IntegrationService` provides a normalized control plane for nine version-pinned Graph, Gmail, Drive, AMS, property, model, and verifier boundaries. Connection configuration stores scoped credential references and rejects inline secret material. Immutable schema versions drive durable one-page/one-batch jobs; attempts retain retry, rate-limit, cursor, and dead-letter evidence; human-confirmed replay appends a successor. Successful provider output is registered only after an exact encrypted receipt is independently read back. Public webhooks require scoped HMAC verification and replay-window checks before exact bytes enter quarantine and a durable job is queued. Fixture health never establishes live connection authority. The authenticated `/integrations` workspace and production routes expose these controls without falling back to `DemoState`. See [INTEGRATIONS.md](./INTEGRATIONS.md).
...
M3 adds authenticated principals, memberships, deny-by-default authorization policies, revocation, support-access controls, and resource-complete attack tests. M4–M10 extend the same boundary through secure storage, imports, documents, playbooks, the replacement property/access/brokerage/source/planning/funding/verification/model/recognition/programme contexts, and their immutable histories. M11 extends it through connection events, schema versions, sync jobs/attempts/receipts, webhook endpoints/deliveries, and provider health. Database attacks cover every current integration relation. Managed-provider configuration, MFA policy enforcement, defense-in-depth RLS evaluation, secrets infrastructure, live vendor operation, and deployment validation remain external/M12 gates rather than inferred passes.
...
- Integration provider/service boundary: `lib/production/integration-providers.ts`, `lib/production/integration-service.ts`
...
The locally implemented M0–M11 tree is a reusable foundation. M12 operational hardening and launch validation is next. Real sponsor/payment operation, authorized independent verification, external model/provider validation, counterparty commitments, live integration semantics, recognition delivery/outcomes, customer/programme use, and operational readiness remain external or incomplete. Production remains closed to customer data until managed PostgreSQL/PostGIS, OIDC/MFA, private storage/scanning, secrets/provider configuration, deployment, backup/restore, security, rights-cleared source/data, and external gates are validated.
