...
export class BrokerageCaseService {
  ...
  private async loadSubmissions(context: TenantContext, caseId: string) {
    const rows = await this.database
      .select({
  ...
  private async caseWorkspace(context: TenantContext, row: CaseRow) {
    const caseId = row.brokerageCase.id;
    assertCaseAccess(context, caseId, [
      { resource: "renewal_case", action: "read" },
      { resource: "policy", action: "read" },
      { resource: "source_document", action: "read" },
      { resource: "document_fact", action: "read" },
      { resource: "evidence_request", action: "read" },
      { resource: "evidence_request_version", action: "read" },
      { resource: "submission", action: "read" },
      { resource: "submission_version", action: "read" },
      { resource: "submission_artifact", action: "read" },
    ]);
    const [locations, notice, requests, evidence, submissions, contradictions] =
      await Promise.all([
        this.loadLocations(context, row.property.id),
        this.loadNoticeBundle(context, caseId),
        this.loadEvidenceRequests(context, caseId),
        this.loadEvidence(context, row.property.id),
        this.loadSubmissions(context, caseId),
        this.database
          .select({ value: count() })
    ...
    return {
      ...
      gates: {
        noticeFactsConfirmed: Boolean(notice.document) && !missingNoticeFacts.length,
        ...
        packetGenerated: submissions.some((submission) =>
          submission.artifacts.some((artifact) => artifact.artifactType === "zip"),
        ),
      },
    };
  }
  ...
  async getWorkspace(context: TenantContext) {
    ...
    const cases = await Promise.all(
      caseRows.map(async ({ id }) =>
        this.caseWorkspace(context, await this.loadCase(context, id)),
      ),
    );
    return { organization: organization[0], cases };
  }
...
export type BrokerageWorkspace = Awaited<
  ReturnType<BrokerageCaseService["getWorkspace"]>
>;
