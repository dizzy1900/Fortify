    ...
    const [sources, versions, reviews, publications, dependencies, alerts] =
      await Promise.all([
        ...
        this.database
          .select()
          .from(schema.governedSourceReviews)
          .where(eq(schema.governedSourceReviews.organizationId, organization))
          .orderBy(desc(schema.governedSourceReviews.reviewedAt)),
        this.database
          .select()
          .from(schema.governedSourcePublications)
          .where(
            eq(schema.governedSourcePublications.organizationId, organization),
          )
        ...
        this.database
          ...
          .from(schema.governedSourceDependencies)
          .where(
            eq(schema.governedSourceDependencies.organizationId, organization),
          )
          .orderBy(desc(schema.governedSourceDependencies.pinnedAt)),
        this.database
          .select()
          .from(schema.sourceChangeAlerts)
          .where(eq(schema.sourceChangeAlerts.organizationId, organization))
