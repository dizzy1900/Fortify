...
import { Archive, Banknote, BarChart3, BookOpenCheck, BriefcaseBusiness, Building2, CalendarDays, ChevronLeft, ChevronRight, ClipboardCheck, ClipboardSignature, FileCheck2, FileKey2, FileSearch, FileText, Gauge, GitBranch, HandCoins, History, Home, KeyRound, LayoutDashboard, Link2, Menu, Network, PackageCheck, RotateCcw, Send, Settings, ShieldCheck, UploadCloud, X } from "lucide-react";
import { useState, type ReactNode } from "react";
import { useDemo } from "./demo-provider";

const nav = [
  ["Portfolio", "/portfolio", LayoutDashboard], ["California brokerage", "/brokerage", BriefcaseBusiness], ["Property graph", "/property-graph", GitBranch], ["Identity & access", "/access", KeyRound], ["SOV import", "/imports", UploadCloud], ["Document intake", "/documents", FileSearch], ["Source register", "/sources", FileKey2], ["Market playbooks", "/playbooks", BookOpenCheck], ["Resilience planning", "/resilience-planning", Banknote], ["Funding & execution", "/funding", HandCoins], ["Independent verification", "/verification", ClipboardSignature], ["Model recognition", "/model-recognition", Network], ["Market recognition", "/recognition", Send], ["Programme intelligence", "/programme-intelligence", BarChart3], ["Integration operations", "/integrations", Link2], ["Community", "/community", Building2], ["Policy timeline", "/policy", CalendarDays], ["Carrier notice", "/notice", FileText], ["Requirement crosswalk", "/requirements", ClipboardCheck], ["Evidence room", "/evidence", Archive], ["Renewal case", "/case", Gauge], ["Packet builder", "/packet", PackageCheck], ["Underwriter review", "/underwriter", ShieldCheck], ["Outcome", "/outcomes", FileCheck2], ["Maintenance", "/maintenance", CalendarDays], ["Reports & audit", "/reports", History], ["Settings", "/settings", Settings],
] as const;
const guide = [
